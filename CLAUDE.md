...
- **Commandes** : `npm ci` · `npm run typecheck` · `npm run typecheck:tests` · `npm run build` (= `gen:md` + `tsc` + `vite build`) · `npm run test:db` (Postgres 16 + pgvector local, voir `tests/db/README.md`) · `npm run dev` (port 5173) · `npm run preview` (4173).
