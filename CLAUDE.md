...
- Téléphone officiel (décision propriétaire 2026-09-18) : 07 82 98 36 44 — appel `tel:+33782983644`, WhatsApp `https://wa.me/33782983644` — source unique src/data/contact.ts (src/lib/whatsapp.ts en dérive ; scripts/gen-markdown.ts → `npm run gen:md` régénère public/*.md). Identité publique : "ClairDossier" (voir I.5). Ancien numéro retiré partout : 04 91 95 90 32 (ne subsiste que dans l'historique Git et les instantanés de baseline datés).
...
- Application mobile (2026-09-16, branche `feature/mobile-app`) : `mobile/` (Expo SDK 57 · React Native 0.86 · expo-router · TypeScript strict) + `packages/core` = logique métier partagée web/mobile (sans dépendance ; le site la réexporte via src/lib/dossier-workspace.ts). Architecture et décisions : docs/mobile-architecture.md. Aucun onglet IA, aucun achat intégré : le produit ne lit pas les documents (CGV) et l'abonnement reste géré par l'équipe.
