      ...
      <div className="pricing-grid">
        {plans.map((plan) => (
          <article key={plan.id} className={`price-card${plan.id === POPULAR_PLAN ? ' popular' : ''}`}>
            ...
            {plan.price === '0 €' ? (
              <Link className="primary-button full" to="/creer-dossier">Commencer gratuitement</Link>
              ...
              >
                {loadingPlan === plan.id ? 'Préparation…' : isStripeConfigured ? 'Choisir cette formule' : 'Paiement bientôt disponible'}
              </button>
            )}
          </article>
        ))}
