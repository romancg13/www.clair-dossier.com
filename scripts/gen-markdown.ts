...
function generateHome(): void {
  const lines: string[] = [
    ...
    '',
    // Texte du hero réel (src/components/sections/Hero.tsx) — ne rien promettre ici que la page ne dit pas.
...
function generateBlogIndex(): void {
  const lines: string[] = [
    ...
    '',
    // Aucun auteur ni relecteur fictif (src/data/authors.ts) : ne pas annoncer de relecture par des tiers.
